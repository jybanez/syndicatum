...
try {
    ...
    Invoke-Compose -Arguments @('stop', $WorkerService)
    try {
        $createRetryEvent = 'MYSQL_PWD="$MYSQL_ROOT_PASSWORD" mysql -uroot "$MYSQL_DATABASE" -e "SET @pid = (SELECT p.id FROM projects p JOIN users u ON u.id = p.owner_user_id WHERE u.username = ''{username}'' LIMIT 1); INSERT INTO message_events_outbox (event_uuid, project_id, event_type, payload_json, available_at, created_at) VALUES (''{retry_uuid}'', @pid, ''acceptance.retry'', ''{}'', UTC_TIMESTAMP(), UTC_TIMESTAMP())"'.Replace('{username}', $workerFixtureUsername).Replace('{retry_uuid}', $retryEventUuid)

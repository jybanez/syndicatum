...
- Existing teams claim pre-existing accounts through `{SYNDICATUM_ORIGIN}/claim` with one-time operator-issued claim codes. The hosted deployment uses `https://syndicatum.wizaya.com`; self-hosted deployments use their own origin.

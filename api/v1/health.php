...
try {
    ...
    $integrations = ['realtime' => ['enabled' => false, 'configured' => false], 'account' => ['enabled' => false, 'configured' => false], 'google' => ['enabled' => false, 'configured' => false]];
    if ($expanded && Db::tableExists($pdo, 'system_settings')) {
        ...
        $integrations['google'] = ['enabled' => (bool) $settings->get('google.enabled'),
            'configured' => $settings->get('google.client_id') !== '' && $settings->get('google.callback_url') !== ''
                && !empty($public['google.client_secret']['configured'])];
    }
    Api::json(['data' => [
        'service' => [
            'id' => 'syndicatum',
            'protocol' => 'syndicatum-connector-v1',
            'api_version' => 'v1',
            'capabilities' => [
                'connector_device_authorization' => true,
                'discussion_bindings' => true,
                'mcp' => true,
            ],
        ],
        'core' => ['status' => 'ok', 'database' => true, 'expanded_schema' => $expanded],

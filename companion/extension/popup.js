...
function renderDeliveryReviews(reviews = []) {
  ...
  for (const review of reviews) {
    ...
    const agentLabel = review.agentName ? `${review.agentName} (agent ${review.agentId})` : `agent ${review.agentId}`;
    title.textContent = `${review.provider || "Provider"} · project ${review.projectId} · ${agentLabel}`;

(function installSyndicatumCompanionListener(root) {
  ...
  function showBindingIntent(intent) {
    ...
    panel.innerHTML = `<h2 id="syndicatum-binding-title" style="margin:0 0 8px;font-size:20px">Confirm Syndicatum discussion binding</h2>
      ...
      <dl style="display:grid;grid-template-columns:130px 1fr;gap:9px;margin:0 0 18px"><dt>Discussion</dt><dd data-field="discussion"></dd><dt>Project</dt><dd data-field="project"></dd><dt>Agent identity</dt><dd data-field="agent"></dd><dt>Agent action</dt><dd data-field="action"></dd></dl>
    ...
    panel.addEventListener("click", async event => {
      ...
      if (action === "verify") {
        await submitBindingStatusCheck(id, panel, host);
        return;
      }
  ...
  async function submitBindingStatusCheck(intentId, panel, host) {
    ...
    if (verification?.ok) { host.remove(); return; }

    description.textContent = "The binding succeeded, but the automatic MCP status check could not be submitted.";
    error.textContent = `Status check pending: ${verification?.message || verification?.code || "submission failed"}. You can retry or close this confirmation.`;
    error.style.display = "block";

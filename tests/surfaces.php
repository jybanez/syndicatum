...
try {
    ...
    $suite->test('Backup and restore actions use canonical Helper components and preserve recovery boundaries', function () use ($suite, $root) {
        ...
        $suite->true(strpos($source, 'invalidateInspectionContext') !== false && strpos($source, 'const completedGeneration = contextGeneration') !== false && strpos($source, 'if (!closed || !contextActive || completedGeneration !== contextGeneration) return;') !== false, 'Dismissed or superseded inspection contexts must not open a stale restore confirmation.');

...
<ul>
    <li><strong>Account and profile information:</strong> display name, email address, password hash when native sign-in is used, profile image, account status, roles, and linked sign-in identifiers.</li>
...
<p>Project content and participant profile information are shared with authorized participants in the same project. When you connect the Syndicatum plugin to ChatGPT or Codex, OpenAI may process the project data returned by the tools and the instructions you send through those products under your OpenAI account and settings. Information may also be processed by infrastructure or integration providers when necessary to operate the service or when an authorized user invokes that integration. We may disclose information when required by law, to protect rights and safety, or during a legitimate service transfer subject to appropriate safeguards. We do not sell personal information.</p>
...
<p>Syndicatum uses session and security cookies needed to keep you signed in, prevent cross-site request forgery, and maintain the service. The optional Syndicatum Companion stores server, authorization, health, and discussion-binding configuration locally in your browser profile so it can route metadata-only notifications to a bound discussion. Authoritative project messages remain on the Syndicatum server and are read through the authenticated plugin tools. Syndicatum does not use these essential technologies for third-party advertising.</p>
...
<h2>Contact</h2>
<p>For hosted-service privacy requests, use the contact options on the public <a href="support">Syndicatum support page</a>. For a self-hosted deployment, contact that deployment's administrator. If you were invited to a project, the project owner or administrator can also direct your request to the appropriate operator.</p>
<?php

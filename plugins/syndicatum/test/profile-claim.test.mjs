...
test("claim action stores a protected per-agent profile outside the project and returns no secrets", async () => {
  ...
  const result = await claimAgentProfile({ syndicatumUrl: "https://chatviewer.pbb.ph", project: "BimoPerks", identity: "Code Planner-Reviewer", claimCode: "one-time-code", projectRoot: root }, { fetchImpl, env, storeTokenImpl: protectedStore });
  ...
  assert.equal(request.url, "https://chatviewer.pbb.ph/api/v1/agent-claim.php");
  assert.equal(result.profileId, agentProfileId("https://chatviewer.pbb.ph", 3, 29));
...
test("two agents sharing one project root receive distinct protected credential files", async () => {
  ...
  const reviewer = await claimAgentProfile({ syndicatumUrl: "https://chatviewer.pbb.ph", project: "BimoPerks", identity: next.identity, claimCode: "reviewer-code", projectRoot: root }, { fetchImpl, env, storeTokenImpl: protectedStore });
  next = { agent: 30, participant: 42, identity: "Codex Developer" };
  const developer = await claimAgentProfile({ syndicatumUrl: "https://chatviewer.pbb.ph", project: "BimoPerks", identity: next.identity, claimCode: "developer-code", projectRoot: root }, { fetchImpl, env, storeTokenImpl: protectedStore });
...
test("a device-token environment override cannot replace a claimed agent credential", async () => {
  ...
  let receivedEnv;
  try {
    const claimed = await claimAgentProfile({ syndicatumUrl: "https://chatviewer.pbb.ph", project: "BimoPerks", identity: "Helper", claimCode: "claim-one", projectRoot: root }, {
...
test("an existing profile blocks claim consumption unless replacement is explicit", async () => {
  ...
  const env = { ...process.env, SYNDICATUM_PLUGIN_DATA: data, SYNDICATUM_AGENT_TOKEN: "" };
  const input = { syndicatumUrl: "https://chatviewer.pbb.ph", project: "BimoPerks", identity: "Code Planner-Reviewer", claimCode: "code", projectRoot: root };
...
test("a complete legacy project credential migrates to the protected profile store and is removed", async () => {
  ...
  await writeFile(legacyFile, JSON.stringify({ project_id: 3, participant_id: 41, agent_id: 29, project_name: "BimoPerks", identity: "Code Planner-Reviewer", token: "legacy-secret", token_prefix: "legacy", claimed_at: "2026-09-13T00:00:00Z", chatviewer_url: "https://chatviewer.pbb.ph" }));
  const migrated = await migrateLegacyProjectCredential(root, env, { storeTokenImpl: protectedStore });
  assert.equal(migrated.profile_id, agentProfileId("https://chatviewer.pbb.ph", 3, 29));

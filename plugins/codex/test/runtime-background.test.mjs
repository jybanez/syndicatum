...
test("MCP runtime leaves listener ownership with a healthy background service", async () => {
  ...
  let ensureCalls = 0;
  const runtime = new PluginRuntime({ SYNDICATUM_PLUGIN_DATA: root, SYNDICATUM_AGENT_TOKEN: "test-token" }, {
    background: {
      async ensureRunning() {
        ensureCalls += 1;
...
test("connector status is rebuilt from persisted configuration instead of stale MCP memory", async () => {
  ...
  const runtime = new PluginRuntime({ SYNDICATUM_PLUGIN_DATA: root, SYNDICATUM_AGENT_TOKEN: "test-token" }, { background: { async status() { return { supported: true, running: true, ownsListener: true, pid: 91, listenerPid: 91, readiness: "ready" }; } } });

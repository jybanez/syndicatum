...
export async function loadConfig(env = process.env) {
  ...
  if (raw.mode === "device") {
    ...
    return Object.freeze({
      ...
      coalescingMaxAgeMs: clamp(raw.coalescingMaxAgeMs, 10000, 300000, 60000),
...
export async function saveDeviceConfig(input, env = process.env) {
  const files = pluginPaths(env);
  const raw = { mode: "device", syndicatumUrl: String(input.syndicatumUrl).trim().replace(/\/+$/, ""), deviceId: String(input.deviceId).trim(), codexPath: String(input.codexPath || "").trim() || null, reconnectDelayMs: 5000, bindingRefreshMs: 15000, coalescingPollMs: 5000, coalescingMaxAgeMs: 60000, activationRetryLimit: 8 };
